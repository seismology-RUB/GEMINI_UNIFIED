...
!--------------------------------------------------------
 module sffDatablock
    ...
    interface modifyTanfSFFDatablock
        module procedure modifyTanfDPSFFDatablock
        module procedure modifyTanfSPSFFDatablock
    end interface
    ...
    interface operator (.tanfdp.); module procedure getTanfDPSFFDatablock; end interface
    ...
    type sff_datablock
        ...
        double precision :: tstart           ! t-value of first data point in sec after midnight
...
!
    call sff_RWid2DP(lu,this%wid,this%n,this%tstart,this%dt,this%nchar,this%ampfac,this%code,ierr)
    if(ierr /= 0) then
...
!------------------------------------------------------------------------------
!  get start time of data block (single)
!
    real function getTanfSFFDataBlock(this)
    type (sff_datablock), intent(in) :: this
    getTanfSFFDataBlock = sngl(this%tstart)
    end function getTanfSFFDataBlock
!------------------------------------------------------------------------------
!  get start time of data block (double)
!
    double precision function getTanfDPSFFDataBlock(this)
    type (sff_datablock), intent(in) :: this
    getTanfDPSFFDataBlock = this%tstart
    end function getTanfDPSFFDataBlock
!------------------------------------------------------------------------------
...
!
    subroutine modifyTanfDPSFFDatablock(this,tstart)
    type (sff_datablock) :: this
    real :: second
...
!
    this%tstart = tstart
    call sff_TimeSplitDP(tstart, day, hour, minute, second)
    call sff_ModWid2time(this%wid, hour, minute, second)
    end subroutine modifyTanfDPSFFDatablock
!------------------------------------------------------------------------------
!  modify tstart of datablock, also change wid2line (single)
!
    subroutine modifyTanfSPSFFDatablock(this,tstart)
    type (sff_datablock) :: this
    real :: tstart
    call modifyTanfDPSFFDatablock(this,dble(tstart))
    end subroutine modifyTanfSPSFFDatablock
!------------------------------------------------------------------------------
...
!
    ...
    real, dimension(:), pointer :: temp => null()
!
