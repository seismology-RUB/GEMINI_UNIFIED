c----------------------------------------------------------------------------
c   Copyright 2016 Thomas Frobriger (Karlsruhe Institute of Technology)
c   Copyright 2016 Wolfgang Friederich (Ruhr-Universitaet Bochum, Germany)
c
c   This file is part of GEMINI_UNIFIED version 1.0.
c
c   GEMINI_UNIFIED version 1.0 is free software: you can redistribute it and/or modify
c   it under the terms of the GNU General Public License as published by
c   the Free Software Foundation, either version 2 of the License, or
c   (at your option) any later version.
c
c   GEMINI_UNIFIED version 1.0 is distributed in the hope that it will be useful,
...
c   V1.10   06/02/01   W.F.   maxbuf auf 1200000 gesetzt
c
cB
c======================================================================
c
...
cD
c----------------------------------------------------------------------
	subroutine sff_NewToc(lu,log,filename,logfile,ierr)
	integer lu,log,ierr
	character filename*(*),logfile*(*)
c-
	call sff_New(lu,filename,ierr)
	open(log, file = logfile)
	close(log, status='delete')
	return
	end
c----------------------------------------------------------------------
...
c
      ...
      character filename*(*), lines(nline)*80
cE
      character code*10
c
      open(lu, file=filename, err=99, status='new')
...
cD
c----------------------------------------------------------------------
c  open sff file for writing with separate table of contents file
c-
	subroutine sff_WOpenToc(lu, log, filename, logfile, ierr)
	integer lu,ierr,log
	character filename*(*),logfile*(*)
c-
	open(log,file=logfile)
	call sff_WOpen(lu,filename,ierr)
	return
	end
c-------------
	subroutine sff_WOpenTocF(lu, log, filename, logfile, lines, nline, ierr)
	integer lu,ierr,log,nline
	character filename*(*),lines(nline)*80,logfile*(*)
c-
	open(log,file=logfile)
	call sff_WOpenF(lu,filename,lines,nline,ierr)
	return
	end
c----------------
	subroutine sff_WOpenTocS(lu, log, filename, logfile, typ, 
     1	                         cs, c1, c2, c3, date, time, ierr)
	integer lu,ierr,log
	real c1,c2,c3
	character filename*(*),date*(*),time*(*),typ*(*),cs*(*),logfile*(*)
c-
	open(log,file=logfile)
	call sff_WOpenS(lu,filename, typ, cs, c1, c2, c3, date, time, ierr)
	return
	end
c------------------
	subroutine sff_WOpenTocFS(lu, log, filename, logfile, lines, nline, 
     1	                          typ, cs, c1, c2, c3, date, time, ierr)
	integer lu,ierr,log,nline
	real c1,c2,c3
	character filename*(*),lines(nline)*80,date*(*),time*(*),typ*(*),cs*(*),logfile*(*)
c-
	open(log,file=logfile)
	call sff_WOpenFS(lu,filename, lines, nline, 
     1	                 typ, cs, c1, c2, c3, date, time, ierr)
	return
	end
c----------------------------------------------------------------------
...
cD
c----------------------------------------------------------------------
      subroutine sff_WTraceNoScale(lu, wid2line, nsamp, fdata, idata, last, ierr)
c
c Write one data block starting with DAST line.
c Data are not scaled to max=2**23, but directly converted to integer
...
c
c workspace:
c   idata           fdata will be converted to idata NOT using sff_f2i
c                   (both array may be in same memory space - see
c                   comments on sff_f2i)
c
      integer lu, nsamp, idata(nsamp),i
      real fdata(nsamp)
      logical last
...
c
      ...
      real fdata(nsamp)
      logical last
      character wid2line*132, lines(nline)*80
cE
      character code*(10)
      real ampfac
      integer ierr
...
cD
c----------------------------------------------------------------------
c  write a trace and write wid2line to a log file
c  useful to know later in advance what's inside the file
c-
	subroutine sff_WTraceToc(lu, log, wid2line, nsamp, fdata, idata, last, ierr)
	integer lu, log, nsamp, idata(nsamp),ierr
	real fdata(nsamp)
	logical last
	character wid2line*(*)
c-
	write(log,'(a68)') wid2line(1:68)
	if(last) close(log)
	call sff_WTrace(lu,wid2line, nsamp, fdata, idata, last, ierr)
	return
	end
c-
	subroutine sff_WTraceTocF(lu, log, wid2line, nsamp, fdata, idata, last,
     1	                          nline, lines, ierr)
	integer lu,log,nsamp,idata(nsamp),nline,ierr
	real fdata(nsamp)
	logical last
	character wid2line*(*), lines(nline)*80
c-
	write(log,'(a68)') wid2line(1:68)
	if(last) close(log)
	call sff_WTraceF(lu,wid2line, nsamp, fdata, idata, last, nline, lines, ierr)
	return
	end
c-
	subroutine sff_WTraceTocI(lu, log, wid2line, nsamp, fdata, idata, last, 
     1	                          cs, c1, c2, c3, nstack, ierr)
	integer lu,log,nsamp,idata(nsamp),ierr,nstack
	real fdata(nsamp),c1,c2,c3
	logical last
	character wid2line*(*),cs*1
c-
	write(log,'(a68)') wid2line(1:68)
	if(last) close(log)
...
c-
	subroutine sff_WTraceTocFI(lu, log, wid2line, nsamp, fdata, idata, last, 
     1	                           nline, lines, cs, c1, c2, c3, nstack, ierr)
	integer lu,log,nsamp,idata(nsamp),ierr,nstack,nline
	real fdata(nsamp),c1,c2,c3
	logical last
	character wid2line*(*),cs*1,lines(nline)*80
c-
	write(log,'(a68)') wid2line(1:68)
	if(last) close(log)
...
cD
c----------------------------------------------------------------------
      subroutine sff_TimeSplitDP(tsec, day, hour, minute, second)
c
...
c----------------------------------------------------------------------
      ...
      integer day, hour, minute
cE
      call sff_TimeSplitDP(dble(tsec), day, hour, minute, second)
      return
      end
cD
c----------------------------------------------------------------------
...
cE
      ...
      integer*4 iarray(3)
      integer jyear, i
      real sff_libversion, version
c
      version=sff_libversion()
      call idate(iarray)
...
c
      write(lu,'(a5,1x,f6.2,1x,a13,1x,a)') 'STAT ',version,timestamp,code
      return
      end
cD
c----------------------------------------------------------------------
...
c
     ...
     1              ncode,' characters'
      ...
      read(lu,'(a5,1x,f6.2,1x,a13,1x,a)', err=101) stat,version,stamp,code
...
cD
c----------------------------------------------------------------------
      subroutine gse_RWid2(lu,wid2line,nsamp,tanf,dt,ierr,last)
c
c  Reads the WID2-line only
c  Input: lu
c  Output
c    wid2line: WID2 line
c    nsamp:    number of samples
c    tanf:     start time in seconds after midnight
c    dt:       sampling interval
c    ierr:     error code
c
      integer lu, ierr, nsamp, last
      character wid2line*(*)
      real tanf, dt
cE
...
c
 10   read(lu,'(a)',end=11) wid2
      if(wid2(1:4).ne.'WID2') goto 10
...
cD
c----------------------------------------------------------------------
      subroutine sff_RWid2DP(lu,wid2line,nsamp,tanf,dt,nchar,ampfac,code,ierr)
c
...
c
      ...
      read(time(4:5),'(i2)') minute
      read(time(7:12),'(f6.3)') second
      tanf = hour*3600.d0 +minute*60.d0 +dble(second)
      if (hour >= 23) tanf = tanf-86400.d0
      goto 99
c
 101  ierr = 1
...
c--------------------------------------------------------------------------
      ...
      real ampfac,dt,tanf
cE
      call sff_RWid2DP(lu,wid2line,nsamp,dble(tanf),dt,nchar,ampfac,code,ierr)
      return
      end
cD
c--------------------------------------------------------------------------
...
cD
c----------------------------------------------------------------------
      subroutine gse_RData(lu,nsamp,idata,ierr)
c
...
c
      integer maxbuff
      parameter(maxbuff = 1 200 000)
      integer lu, ierr, nsamp, idata(nsamp),nchar
      character cbuf(maxbuff)*1
cE
      character line*80
      integer i, j, ichecksum, nchecksum, nmax, ierror
c
c  search for DAT2 line
c
 13   read(lu,'(a)') line
      if(line(1:4).ne.'DAT2') goto 13 
c
