! ======================================================================
!  Bulirsch-Stoer integration engine for real-valued solution vectors
! ======================================================================
!----------------------------------------------------------------------------
...
!  Here version for double precision solution vector
!------------------------------------------------------------------
module realBulirschStep
    use parametersBulirschStep
    use baseStepEngine
    implicit none
    type, extends(base_step_engine) :: real_bulirsch_step       ! extends base class
       private
       integer :: donetry                                       ! number of done subdivision attempts
       double precision, dimension(:,:), allocatable :: ysol    ! sequence of solution vectors for all
                                                                ! attempted  subdivision levels
       double precision, dimension(:), allocatable :: yext        ! final extrapolated solution
       double precision, dimension(:), allocatable :: yerr        ! error of final extrapolated solution
       double precision, dimension(:,:), allocatable :: rhs       ! values of right hand side for all 
                                                                ! attempted subdivisions (2nd index)
       double precision, dimension(:,:,:), allocatable :: sm      ! values of system matrix for all 
                                                                ! attempted subdivisions (last index)
    contains
       procedure :: create => createRealBulirsch             ! mapping of base class procedures 
       procedure :: dealloc => deallocRealBulirsch           ! concrete implementations
       procedure :: doFullStep => doFullStepRealBulirsch
       procedure :: adjustStepSize => adjustStepSizeRealBulirsch
       procedure :: getSolution => getSolutionRealBulirsch
       procedure :: doLinearModifiedMidpointRealBulirsch
       procedure :: doNonLinearModifiedMidpointRealBulirsch
       procedure :: extrapolateRealBulirsch
       procedure :: printResultRealBulirsch
    end type real_bulirsch_step
!
contains
!--------------------------------------------------------------------
!> \brief Create a bulirschStep object with real variables
!   sode:     polymorphic object representing ODS system
!   eps:      desired accuracy
!   secrecy:  anti-debugging level
!
    subroutine createRealBulirsch(this,sode,eps,secrecy)
    class (real_bulirsch_step) :: this
    class (base_ode_system), target :: sode
    double precision :: eps
    integer :: secrecy
    character(len=21) :: myname = 'createRealBulirsch'
!
    ...
    end subroutine createRealBulirsch
!-----------------------------------------------------------------------------------
!> \brief Deallocate object
!
    subroutine deallocRealBulirsch(this)
    class (real_bulirsch_step) :: this
!
    ...
    end subroutine deallocRealBulirsch
!--------------------------------------------------------------------------
...
!  yi:     imaginary part of initial solution vector (in) (not used)
!
    subroutine doFullStepRealBulirsch(this,h,xstart,yr,yi)
    class (real_bulirsch_step) :: this
    double precision :: h,xstart
    double precision, dimension(:) :: yr,yi
    integer :: nfirst,ntry,j
    double precision :: errmax
    double precision, dimension(:), allocatable :: zyy,zdy
!
...
1   do j = nfirst,ntry
       if (this%sode%linear) then
          call this%doLinearModifiedMidpointRealBulirsch(j,h,xstart,yr,yi)
       else
          call this%doNonLinearModifiedMidpointRealBulirsch(j,h,xstart,yr,yi)
       endif
    enddo
!
!  extrapolate solution to step size zero
!
    call this%extrapolateRealBulirsch(h,zyy,zdy)
    errmax = sqrt(sum(abs(zdy)**2)/sum(abs(zyy)**2))         ! length ratio of error vector and solution vector
!    errmax = maxval(abs(zdy)/abs(zyy))
    if (this%secrecy <= secrecy_bulirsch_step) call this%printResultRealBulirsch(xstart,h,errmax,zyy)
...
!
    if (ntry < maxtry_bulirsch_step) then                   ! try more subdivisions of same step
       ...
       if (this%secrecy <= secrecy_bulirsch_step) then
          print *,'doFullStepRealBulirsch: xstart,errmax = ',xstart,errmax
    ...
    end subroutine doFullStepRealBulirsch
!---------------------------------------------------------------------------------------------------------------
...
!  yr:       real part of initial solution vector (in)
!  yr:       imaginary part of initial solution vector (in) (not used)
!  No error checking here for efficiency reasons
!
    subroutine doLinearModifiedMidpointRealBulirsch(this,jtry,h,xstart,yr,yi)
    class (real_bulirsch_step) :: this
    integer :: jtry
    double precision :: h,xstart
    double precision, dimension(:) :: yr,yi
    double precision, dimension(:), allocatable :: z0,z1,z2
    integer :: j,ix,nstep,m,ios1,ios2
    double precision :: x
    double precision :: hdiv
!    double precision :: hdiv,dcone = dcmplx(1.d0,0.d0)            ! required to use BLAS
!
...
!
    do j = 1,nxdiv_bulirsch_step(jtry)
       ...
       call this%sode%getSysmatReal(x,this%sm(:,:,ix),ios2)
       call this%sode%getRhsReal(x,this%rhs(:,ix),ios1)
    enddo
!
...
!    call zcopy(this%nvar,z2,1,this%ysol(:,jtry),1)                                         ! ysol = z2
    ...
    deallocate(z0,z1,z2)
    this%donetry = jtry
    end subroutine doLinearModifiedMidpointRealBulirsch
!---------------------------------------------------------------------------------------------------------------
...
!  yr:       imaginary part of initial solution vector (in) (not used)
!
    subroutine doNonLinearModifiedMidpointRealBulirsch(this,jtry,h,xstart,yr,yi)
    class (real_bulirsch_step) :: this
    integer :: jtry
    double precision :: h,xstart
    double precision, dimension(:) :: yr,yi
    double precision, dimension(:), allocatable :: z0,z1,z2
    double precision, dimension(:), allocatable :: zderiv
...
!
    ...
    z0 = yr
    x = xstart
    call this%sode%derivativeReal(x,z0,zderiv,ios1)
    z1 = z0+hdiv*zderiv
    do m = 1,nstep-1
       x = xstart+m*hdiv
       call this%sode%derivativeReal(x,z1,zderiv,ios2)
       z2 = z0+2.*hdiv*zderiv
       z0 = z1; z1 = z2
    enddo
    x = xstart+h
    call this%sode%derivativeReal(x,z1,zderiv,ios3)
    this%ysol(:,jtry) = 0.5*(z1+z0+hdiv*zderiv)
    deallocate(z0,z1,z2,zderiv)
    this%donetry = jtry
    end subroutine doNonLinearModifiedMidpointRealBulirsch
!---------------------------------------------------------------
...
!------------------------------------------------------------------------
    subroutine extrapolateRealBulirsch(this,hs,yy,dy)
    class (real_bulirsch_step) :: this
    double precision :: hs
    double precision, dimension(:) :: yy,dy                  ! dimension = nvar
...
!
    ...
    enddo
    deallocate(c,d,x,u)
    end subroutine extrapolateRealBulirsch
!---------------------------------------------------------------------
!> \brief Adjust step size
!  h:       step size (inout)
!  success: flag fo success of integration step (in)
!
    subroutine adjustStepSizeRealBulirsch(this,h,success)
    class (real_bulirsch_step) :: this
    double precision :: h
    logical :: success
    if (success) then
    ...
    end subroutine adjustStepSizeRealBulirsch
!---------------------------------------------------------------------
!> \brief Get solution at end of step (real)
!
    subroutine getSolutionRealBulirsch(this,yr,yi)
    class (real_bulirsch_step) :: this
    double precision, dimension(:) :: yr,yi
    yr = this%yext
    end subroutine getSolutionRealBulirsch
!--------------------------------------------------------------
!> \brief print results from each subdivision step
!
    subroutine printResultRealBulirsch(this,xstart,h,errmax,zy)
    class (real_bulirsch_step) :: this
    double precision :: xstart,h,errmax
    double precision, dimension(:) :: zy
    print *,'Result of certain subdivision of real Bulirsch Step'
    print *,'xstart: ',xstart
    print *,'h:      ',h
    print *,'ntry:   ',this%donetry
    print *,'err:    ',errmax
    print *,'ysol =  ',zy
    end subroutine printResultRealBulirsch
!
end module realBulirschStep
