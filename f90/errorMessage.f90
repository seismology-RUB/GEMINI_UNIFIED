...
!
    ...
    string = trim(string)//' in '//trim(fctname)//new_line('a')//trim(message)
    if (associated(this%messages)) then
...
!---------------------------------------------------------------
!> \brief Reduce level of error message by 1
!
    subroutine reduceLevelErrorMessage(this)
    type (error_message) :: this
    this%level = max(0,this%level-1)
    end subroutine reduceLevelErrorMessage
!---------------------------------------------------------------
