...
module argumentParser
    ...
    integer, parameter :: max_length_option = 50
    integer, parameter :: max_length_posnam = 50
...
!
    subroutine documentArgumentParser(this,lulog)
    type (argument_parser) :: this
...
!
    ...
    write(lu,'(80(1h-))')
    write(lu,'(25x,41a)') '| DOCUMENTATION OF COMMANDLINE CONTENTS |'
    write(lu,'(25x,41(1h-))')
    write(lu,'(a9,4x,a)') 'Program: ',trim(this%progname)
    write(lu,'(a13,a)') 'Description: ',trim(this%description)
    write(lu,'(1x)')
    write(lu,'(a)') 'Positional arguments:'
    do j = 1,this%npos
       write(lu,'(a24,a9,a5,a10,a)') trim(this%posnam(j)),': Type = ',trim(this%postyp(j)),', Value = ',trim(this%posarg(j))
    enddo
    write(lu,'(1x)')
    write(lu,'(a)') 'Optional arguments:'
    do j = 1,this%nopt
       if (.not.this%optset(j)) then
          write(lu,'(a14,a9,a5,a12,a)') trim(this%option(j)),': Type = ',trim(this%opttyp(j)),', Default = ',trim(this%optdef(j))
       else
          write(lu,'(a14,a9,a5,a12,a)') trim(this%option(j)),': Type = ',trim(this%opttyp(j)),', Value   = ',trim(this%optarg(j))
       endif
    enddo
    write(lu,'(80(1h-))')
