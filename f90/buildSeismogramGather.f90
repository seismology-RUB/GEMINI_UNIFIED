! ===============================================================================
!  Main program for building a seismogram gather
! ===============================================================================
!----------------------------------------------------------------------------
...
!
   ...
   !
       do while (nextEventSeismicEventList(event_list,event))
          gatherfile = path_synthetic_seis+(.evid.event)+'_gather.asc'
   ...
   !
       ...
       !
                call writeAsciiSynseisIO(2,gatherfile,nsamp,tanfdp,dt,staname,component,urs,ios,op = do_open,cl = do_close)
                if (ios > 0) then
                   call add(errmsg,2,'Synthetic file could not be opened ->'+trim(gatherfile),myname)
                   call print(errmsg); stop
                else if (ios < 0) then
                   call add(errmsg,2,'End of file reached ->'+trim(text),myname)
                   call print(errmsg); stop
                endif
                deallocate(urs); do_open = .false.
             enddo
          enddo
          close(2)                           ! close gather file manually here
       enddo
    case default
...
!
 ...
 end program buildSeismogramGather
