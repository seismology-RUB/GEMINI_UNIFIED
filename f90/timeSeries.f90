...
!-------------------------------------------------------------
 module timeSeries
    ...
    type time_series
        ...
        double precision :: dt                       !< Sampling interval
...
!
    ...
    integer :: nsamp
    real :: dt,tanf
    call createDPZeroTimeSeries(this,nsamp,dble(tanf),dble(dt))
...
!
    ...
    real :: dt,tanf
    real, dimension(:), target :: s
    call createDPLinkTimeSeries(this,nsamp,dble(tanf),dble(dt),s)
...
!
    ...
    real :: dt,tanf
    real, dimension(:) :: s
    call createDPFromDataTimeSeries(this,nsamp,dble(tanf),dble(dt),s)
...
!-------------------------------------------------------------
!> \brief Create a time series with empty (unassociated) data vector
!> \param this Time series object
...
!
    ...
    type (time_series) :: this
    real :: dt,tanf
    call createDPEmptyTimeSeries(this,dble(tanf),dble(dt))
...
!
    ...
    real :: dtnew
    real :: therd,tbh,tah
    ats = adjustDPTimeSeries(this,dble(therd),dble(tbh),dble(tah),dble(dtnew))
...
!-------------------------------------------------------------------------
!> \brief downsampling of time series without anti-alias filtering
!
    function downsampleTimeSeries(this,ndec) result(rts)
    type (time_series) :: this,rts
    integer :: ndec
    double precision :: dt
    integer :: nsamp,k,i
!
    nsamp = (this%nsamp-1)/ndec+1
    dt = this%dt*ndec
    if (this%exdate) then
        call createZeroTimeSeries(rts,nsamp,this%stime,dt)
    else
        call createZeroTimeSeries(rts,nsamp,this%tanf,dt)
    endif
    k = 1
    do i = 1,this%nsamp,ndec
       rts%y(k) = this%y(i)
       k = k+1
    enddo
    end function downsampleTimeSeries
!-------------------------------------------------------
!> \brief  get dt of time series
!
    double precision function dtTimeSeries(this)
...
!
    ...
    type (time_series), intent(in) :: this
    real :: tend
    tend = real(this%tanf)+(this%nsamp-1)*this%dt
...
!
    double precision function energyUnnormalizedTimeSeries(this,n1,n2)
    type (time_series) :: this
    ...
    if (present(n1)) na = n1
    if (present(n2)) ne = n2
    energyUnnormalizedTimeSeries = sum(this%y(na:ne)**2)
    end function energyUnnormalizedTimeSeries
!--------------------------------------------------------
!> \brief  calculate envelope of time series
!>  return envelope time series
!
    function envelopeTimeSeries(this) result(env)
    type (time_series) :: this,env
    integer :: nlog,n,n1,i
    double precision, dimension(:), allocatable :: e,work
...
!
    ...
    e(1:n1) = this%y(1:n1)
    e(n1+1:n) = 0.
    call fastFourierTransform(e,work,nlog,-1)
!
...
!
    call fastFourierTransform(e,work,nlog,+2)
    if (this%exdate) then
        call createZeroTimeSeries(env,n1,this%stime,this%dt)
    else
        call createZeroTimeSeries(env,n1,this%tanf,this%dt)
    endif
    do i=1,n1
        env%y(i)=2.*pythag(e(i),work(i))
    enddo
    deallocate(work,e)
    end function envelopeTimeSeries
!---------------------------------------------------------------------------
...
!-----------------------------------------------------------------------------
!> \brief  linear interpolation of time series at given regularly spaced times
!  tanf:  start of new times
!  dt:    new sampling interval
!  nsamp: number of new time samples
!  its: interpolated time series is restricted to overlapping range of original time series and new time point
!
    function interpolateTimeSeries(this,nsamp,tanf,dt,errmsg) result(its)
    type (time_series) :: this,its
    double precision :: tanf,dt
    integer :: nsamp
    type (error_message) :: errmsg
    integer :: n,i,j
    double precision :: a,b,t,ta,te
    character (len=21) :: myname = "interpolateTimeSeries"
!
!   determine overlapping range of new time points and original time series
!   restrict interpolated time points to this range (ta <= t <= te)
!
    ta = max(tanf,this%tanf)
    te = min(tanf+(nsamp-1)*dt,this%tanf+(this%nsamp-1)*this%dt)
    if (te <= ta) then
       call add(errmsg,2,'new time points do not overlap with time series',myname)
       return
    endif
!
!   true start time of its is tanf+(n-1)*dt >= ta => (n-1)*dt >= ta-tanf => n >= (ta-tanf)/dt+1
!
    n = ceiling((ta-tanf)/dt+1.d0)
    its%tanf = tanf+(n-1)*dt
!
!   true end time of its is its%tanf+(n-1)*dt <= te => (n-1)*dt <= te-its%tanf => n <= (te-its%tanf)/dt+1
!   n = its%nsamp
!
    its%nsamp = floor((te-its%tanf)/dt+1.d0)
    if (its%nsamp < 1) then
       call add(errmsg,2,'new time points do not overlap with time series',myname)
       return
    endif
    its%dt = dt
    its%link = .false.
    its%stime = this%stime.plus.(ta-this%tanf)
    its%exdate = this%exdate
    allocate(its%y(its%nsamp))
!
!  interpolation (this%tanf+(j-1)*this%dt <= t < this%tanf+j*this%dt)
!  j <= (t-this%tanf)/this%dt+1
!
    do i = 1,its%nsamp
       t = its%tanf+(i-1)*dt
       j = floor((t-this%tanf)/this%dt+1.d0)
       a = (this%tanf+j*this%dt-t)/this%dt
       b = 1.d0-a
       if (j == this%nsamp) then   ! j-th sample is last one, (j+1)-th sample does not exist
          its%y(i) = this%y(j)
       else
          its%y(i) = a*this%y(j)+b*this%y(j+1)   ! includes case if t = this%tanf with b=0
       endif
    enddo
!
    end function interpolateTimeSeries
!--------------------------------------------------------
...
!
    ...
    type (time_series), intent(in) :: this
    startTimeSeries = real(this%tanf)
...
!---------------------------------------------------------
!> \brief  Calculate sigma from envelope of time series
!>  using water level and magnification and steepness parameter
!!  see envsigma.f in wolib
!<  return allocated pointer
!
    function sigmaEnvelopeTimeSeries(this,mag,nexp) result(sigma)
    type (time_series) :: this,env
    real, dimension(:), pointer :: sigma
    integer :: nexp,i
    real :: mag,emax,w,x
!
    env = envelopeTimeSeries(this)
    emax=maxval(env%y)
    w=emax/mag
    allocate(sigma(this%nsamp))
    do i=1,this%nsamp
    ...
    enddo
    call dealloc(env)
    end function sigmaEnvelopeTimeSeries
!--------------------------------------------------------
...
!-----------------------------------------------------------------------------
!> \brief  trim time series to given start and end time
!   select closest sample just before start
!   and closest sample just after tend
!
    function trimTimeSeries(this,tbeg,tend,errmsg) result(tts)
    type (time_series) :: this,tts
    double precision :: tbeg,tend
    type (error_message) :: errmsg
    integer :: ib,ie,nsamp
    character (len=14) :: myname = 'trimTimeSeries'
!
!  check if time series contains desired time window
!
    if (this%tanf > tbeg .or. (.tenddp.this) < tend) then
       call add(errmsg,2,'time series either starts after tbeg or ends before tend',myname)
       print *,"Start of time series: ",this%tanf, " End of time series: ",.tend.this
       print *,"Desired trimming to: ",tbeg,tend
       return
    endif
!
!  find index of first and index of last sample
!  this%tanf+(ib-1)*this%dt <= tbeg  ---> ib <= (tbeg-this%tanf)/this%dt+1.0
!  this%tanf+(ie-1)*this%dt >= tend  ---> ie >= (tend-this%tanf)/this%dt+1.0
!
    ib = floor((tbeg-this%tanf)/this%dt+1.d0)
    ie = ceiling((tend-this%tanf)/this%dt+1.d0)
    nsamp = ie-ib+1
    if (this%exdate) then
        call createZeroTimeSeries(tts,nsamp,this%stime,this%dt)
        tts%stime = (this%stime).plus.(dble((ib-1)*this%dt))
    else
        call createZeroTimeSeries(tts,nsamp,this%tanf,this%dt)
    endif
    tts%tanf = this%tanf+(ib-1)*this%dt
    tts%y = this%y(ib:ie)
    end function trimTimeSeries
!-----------------------------------------------------------------------------
...
!-------------------------------------------------------------------------------------
!  write time series to ascii file
!  first column: times
!  second column: values
!
    subroutine writeAsciiTimeSeries(this,lu,filename,errmsg)
    type (time_series) :: this
    integer :: lu
    ...
    integer :: j
!
    allocate(p(this%nsamp,2))
    do j = 1,this%nsamp
       p(j,1) = this%tanf+(j-1)*this%dt
       p(j,2) = this%y(j)
    enddo
    call writeRealMatrixAsciiDataIO(filename,lu,p,errmsg)
    end subroutine writeAsciiTimeSeries
!
