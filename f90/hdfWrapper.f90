...
!  res: returned real value
!  errmsg: error message
!
    subroutine readRealScalarHDFWrapper(locid,dsetname,res,errmsg)
    integer(hid_t) :: locid
    character (len=*) :: dsetname
    real :: res
    type (error_message) :: errmsg

    character (len=22) :: myname = "readRealScalarHDFWrapper"
    integer :: ierr
...
!
!  allocate dummy dims array
!
    allocate(dims(1))
    dims(1) = 1
!
!  read real scalar from HDF according to data type
!
    call h5dread_f(dset,typ,res,dims,ierr)
...
!
 ...
 1  call add(errmsg,2,trim(errstr),myname)
    end subroutine readRealScalarHDFWrapper
!---------------------------------------------------------------------
!                                              READ ARRAY DATA    
!--------------------------------------------------------------------
...
!
    ...
    integer(hid_t) :: dsp,dset,xfer,wsp
    character(len=max_length_string) :: errstr
!
...
!       
    select type (ara)
    class is (any_rank_real_array)
       if (.not. present(offset)) call h5dcreate_f(locid,dsetname,H5T_NATIVE_REAL,dsp,dset,ierr)
       call writeRealArrayHDFWrapper(dset,H5T_NATIVE_REAL,ara,dims,xfer,ierr,wsp,dsp)
    class is (any_rank_integer_array)
    ...
    if (ierr < 0) then; errstr = 'h5dwrite '//trim(dsetname); goto 1; endif
    deallocate(dims)
!
!  close hdf objects
!
