...
!
  subroutine createEventsFromStandardEventFile(event_file,lu,events,errmsg)
    ...
    real :: slat,slon,sdepth,mag,timeshift
    integer :: istyp
    real, dimension(3) :: f
    real, dimension(6) :: M
...
!
    allocate(events(nev))
!    
    ! reads in lines of the form:  eventid  origintime  slat  slon  sdepth  mag  istyp  mom/frce
    open(unit=lu, file=trim(event_file), status = 'old', action = 'read', iostat = ios)
    if (ios /= 0) then
    ...
    do while(ios == 0)
       ...
       if( len_trim(line) > 0 ) then
          ...
          read(line,*) evid,otime_string,slat,slon,sdepth,mag,timeshift,istyp
          call createFromFullTimestringDateTime(otime,otime_string)
          if(istyp==0) then
             read(line,*) evid,otime_string,slat,slon,sdepth,mag,timeshift,istyp,f
             call createForceMomentSeismicEvent(events(iev),evid,csys,slat,slon,sdepth,mag,otime,timeshift,istyp,f)
          elseif(istyp==1) then
             read(line,*) evid,otime_string,slat,slon,sdepth,mag,timeshift,istyp,M
             call createForceMomentSeismicEvent(events(iev),evid,csys,slat,slon,sdepth,mag,otime,timeshift,istyp,M)
          else
             read(line,*) evid,otime_string,slat,slon,sdepth,mag,timeshift
             call createSeismicEvent(events(iev),evid,csys,slat,slon,sdepth,mag,otime,timeshift)
          end if ! istyp==0
